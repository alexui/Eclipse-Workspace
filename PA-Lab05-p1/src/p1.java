...
class Nim {
    ...
    public boolean apply_move(Move move) {
         ...
         */
    	if (move.amount < 1 || move.amount > 3) return false;
    	if (move.heap < 0 || move.heap > 2) return false;
    	
    	for(int i=0;i<heaps.length;i++)
    	{
    		if(move.heap==i && heaps[i]>=move.amount){
    			heaps[i]-=move.amount;
    			return true;
    			}
    	}
...
class P1 {
    ...
    public static Pair<Integer, Move> minimax(Nim init, int player, int depth) {
		...
		for ( Move i: moves)
		{
			...
			clona.apply_move(i);
			Pair<Integer, Move> score = minimax(clona, player, depth - 1);
			score.first = 0 - score.first; // -negamax
			if( score.first >= max ) {
				max = score.first;
				mutare = i;
			}	
		...
		return new Pair<Integer, Move>(max, mutare);
