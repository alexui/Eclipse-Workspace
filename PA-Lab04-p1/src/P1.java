...
public class P1 {
	...
	public static void doBKT(int[][] grid, int row, int column) {
        ...
        if(grid[row][column]==0){	//daca spatiul este gol putem completa cu valori de la 1 la 9
        	
        	for(int i=1;i<=9;i++)
        	{
        		grid[row][column]= i;
        		        		
        		if(isValid(grid, row, column)){	//daca am completat cu numar valid se va reapela metoda 
        	
        			if(column!=grid[row].length-1)		//pentru spatiul din dreapta
        				doBKT(grid,row,column+1);
        			else
        				if(row!=grid.length-1)			//sau pentru primul spatiu de pe randul urmator
        					doBKT(grid, row+1, 0);
        				else 
        					{
        					printGrid(grid);
        		...
        		grid[row][column]=0;	//! trebuie initializat la spatiu vid pentru a permite efectuarea altor cautari
        	}
        }	
        //daca spatiul nu este gol 
        else{
