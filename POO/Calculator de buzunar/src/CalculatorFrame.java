...
public class CalculatorFrame extends JFrame{
	...
	public CalculatorFrame(){
		...
		runCalculator = new ExpressionParser();		//instance of the class that executes the numerical calculations
		expr = new String();
	...
	public class CPanel extends JPanel {//main panel
		...
		CalculatorFrame.this.addKeyListener(ka);
		...
		ActionListener listener = new EventListener(); //listener for each buton
	...
	public class EventListener implements ActionListener{
		...
		public void actionPerformed(ActionEvent e) {
			...
			if(button==log){
				...
				if(expr.isEmpty() || expr.trim().endsWith("("))
					token = "-";
			}
				...
				try {												//eval from ExpressionParser can throw two exceptions
					screen.setForeground(Color.red);
					Double result = runCalculator.eval(expr);
					...
					expr="";
					if(token.equals("Infinity"))					//in case the result cannot be represented on double
					{
					...
					expr += "\nIllegal expression.";
					token=new String();						//to prevent entering the last 'if' block
					screen.setText(expr);
					expr = new String();				//the expression can no longer be modified and it is initialised
				...
				token = ".";			}	
			else if(button==backSpace){								//removes the last character
				if(!expr.isEmpty()){
					expr = expr.substring(0, expr.length()-1);
					screen.setText(expr);}
			}
			else if(button==clear){									//removes the last token
				if(!expr.isEmpty()){
					if(!expr.trim().contains(" "))
						expr="";
					else
						{
						expr=expr.substring(0,expr.trim().lastIndexOf(" "));
						expr +=" ";
						}
					screen.setText(expr);}
				}
			else if(button==ce){									//initialise expresion
				expr = new String();	
				screen.setText(expr);}
